      ...
      <div className="w-full flex-1 flex flex-col justify-between min-h-0 gap-3 sm:gap-4">
        ...
        <div className="w-full flex-1 flex flex-row justify-between items-stretch min-h-0 gap-6">
          ...
          <motion.section 
            ...
            className={`hidden md:flex w-56 sm:w-60 md:w-64 bg-forest-dark/30 border border-forest-secondary/40 rounded-xl p-2.5 backdrop-blur-md shadow-2xl flex-col overflow-hidden flex-shrink-0 transition-all duration-300 ${
              isPlaylistExpanded ? "h-full self-stretch" : "h-fit self-start"
            }`}
          >
            <div 
              onClick={() => setIsPlaylistExpanded(!isPlaylistExpanded)}
              className={`flex items-center justify-between pb-1.5 px-0.5 flex-shrink-0 cursor-pointer select-none group/header transition-colors ${
                isPlaylistExpanded ? "border-b border-forest-secondary/40" : ""
              }`}
            >
              <div className="flex items-center gap-1 font-serif text-[9px] text-zinc-400 tracking-widest font-semibold uppercase group-hover/header:text-gold-khaki transition-colors">
                <Music className="w-3 h-3 text-gold-khaki/70" />
                <span>Curated Sessions</span>
              </div>
              <div className="flex items-center gap-1">
                <span className="font-sans text-[8px] text-zinc-500 tracking-wider font-medium uppercase mr-1">
                ...
                <button
                  ...
                  className={`p-0.5 rounded transition-all duration-200 ${
                    isPlaylistExpanded 
                      ? "text-zinc-400 hover:text-gold-khaki hover:bg-forest-secondary/40 cursor-pointer active:scale-90" 
                      : "text-zinc-600/30 cursor-not-allowed opacity-20"
                  }`}
                  title="Minimize"
                >
